...
func (s *RoomService) CreateRoom(ctx context.Context, req *v1.CreateRoomRequest) (*v1.Room, error) {
	rt, err := s.uc.CreateRoom(ctx, convertRoom(req.GetRoom()))
	if err != nil {
		return nil, err
	}
	return convertRoomReply(rt), nil
}
...
func (s *RoomService) UpdateRoom(ctx context.Context, req *v1.UpdateRoomRequest) (*v1.Room, error) {
	...
	current, err := s.GetRoom(ctx, &v1.GetRoomRequest{RoomId: req.GetRoom().GetRoomId()})
	if err != nil {
		return nil, err
	}

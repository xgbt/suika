...
- **状态**：已采纳（2026-08-25，ADR-0006）——录制泵按块缓冲裁决，
  指纹用 FNV-1a（不含时间戳），重复块整块丢弃，连续 10 块重复包装为
  `ErrStreamTransient` 断开；块边界兜底 25s 时间戳间隔 / 60s 跨度 /
  64MB 缓冲上限。
...
- **状态**：已采纳（2026-08-25，ADR-0005）——新段等待首个视频关键帧，
  关键帧前正文标签丢弃、头标签经缓存注入；音频流豁免；注入头标签计入
  写入字节统计。
...
1. **(7) 排除非 avc codec** — 小时级，消除整类不确定性（已完成，
   ADR-0004）；
2. **(3) 首段关键帧对齐** — 半天，可播放性直接收益（已完成，
   ADR-0005）；
3. **(4) 磁盘防护** — 一天，防全盘故障；
4. **(1) 时间戳修复** — 两三天 + 测试，高频问题；
5. **(2) 重复数据指纹** — 一天，防御 CDN 病态节点（已完成，
   ADR-0006）；

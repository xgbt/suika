...
**Room**:
A Bilibili live room under management, identified by the platform's `room_id`. Carries persisted identity (streamer name, title, `record_enabled` flag — whether the room is configured to be recorded) plus mutable live and recording runtime state.
